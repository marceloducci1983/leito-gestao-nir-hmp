-- Adicionar novo valor ao ENUM discharge_type
ALTER TYPE discharge_type ADD VALUE IF NOT EXISTS 'ALTA A PEDIDO';

-- Atualizar função para mapear valores do modal para ENUM
...
DECLARE
  ...
  mapped_discharge_type discharge_type;
BEGIN
  -- Mapear valores do modal para valores do ENUM
  mapped_discharge_type := CASE p_discharge_type
    WHEN 'ALTA MELHORADA' THEN 'POR MELHORA'
    WHEN 'ALTA A PEDIDO' THEN 'ALTA A PEDIDO'
    WHEN 'EVASÃO' THEN 'EVASÃO'
    WHEN 'TRANSFERÊNCIA' THEN 'TRANSFERENCIA'
    WHEN 'ÓBITO' THEN 'OBITO'
    ELSE 'POR MELHORA' -- valor padrão
  END;
  ...
  IF FOUND THEN
    -- Mover paciente para tabela de altas com tipo de alta mapeado
